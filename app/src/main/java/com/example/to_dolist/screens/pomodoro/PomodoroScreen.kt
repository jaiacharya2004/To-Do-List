...
fun PomodoroScreen(navController: NavHostController, pomodoroViewModel: PomodoroViewModel) {
    ...
    val totalDuration = pomodoroViewModel.selectedTimeInMinutes * 60 * 1000 // Convert selected minutes to milliseconds
    ...
    Column(
        ...
        Box(
            ...
            Canvas(modifier = Modifier.fillMaxSize()) {
                ...
                val sweepAngle = if (isTimerRunning || isPaused) {
                    (timeLeft.toFloat() / totalDuration) * 360f // Remaining progress (fill out)
                } else {
        ...
        Text(
            text = "Set Timer Duration: ${pomodoroViewModel.selectedTimeInMinutes} min",
            color = Color.White,
        ...
        Slider(
            value = pomodoroViewModel.selectedTimeInMinutes.toFloat(),
            onValueChange = { newValue ->
                pomodoroViewModel.selectedTimeInMinutes = newValue.toInt()

                // Restart timer if it's running or paused
                if (pomodoroViewModel.isTimerRunning || pomodoroViewModel.isPaused) {
                    pomodoroViewModel.restartTimer()
                }
            },
            valueRange = 1f..90f,
            steps = 89,
        ...
        if (!isTimerRunning && !isPaused) {
            ...
            // Show "Pause" and "Restart" buttons when timer is running
            Row(
            ...
            // Show "Resume" and "Restart" buttons when timer is paused
            Row(
                horizontalArrangement = Arrangement.Center,
                modifier = Modifier.fillMaxWidth()
            ) {
